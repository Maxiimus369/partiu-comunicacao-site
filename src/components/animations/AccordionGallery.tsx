...
const AccordionGallery = ({
  ...
  const applyLayout = useCallback(
    (animate: boolean) => {
      ...
      panels.forEach((panel, i) => {
        ...
        if (isMobile) {
          tl.to(panel, { minHeight: isActive ? mediaSize : 84, flexGrow: 0, ...rotProp, duration: dur, ease }, 0);
        } else {
          tl.to(panel, { flexGrow: isActive ? grow : 1, minHeight: 0, ...rotProp, duration: dur, ease }, 0);
        }
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      const finalSize = typeof window !== 'undefined' && window.innerWidth <= 520 ? 280 : size;
      
      mediaSizeRef.current = finalSize;
      el.style.setProperty('--ag-media-size', `${finalSize}px`);
